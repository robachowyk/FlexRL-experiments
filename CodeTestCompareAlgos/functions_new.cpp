...
// [[Rcpp::export]]
double JorisFunctionRandomNumber() {
...
// [[Rcpp::export]]
double JorisFunctionRcppSample(IntegerVector choiceset, NumericVector probavec) {
  // std::cout << "probavec" << probavec << std::endl;
  double generatedrandom = Rcpp::sample(choiceset, 1, false, probavec)[0];
  // std::cout << "generatedrandom" << generatedrandom << std::endl;
             ...
             LogicalVector sumColD)
{
  ...
  for (int q = 0; q < S.nrow(); q++)
  {
    ...
    // std::cout << "i,j " << sumRowD(i) << std::endl;
    // std::cout << "i,j " << sumColD(j) << std::endl;
    ...
    {					
      ...
      // Random number smaller than prob -> generate binomial value
      bool link = JorisFunctionRandomNumber() < pswitch;// dist(gen) < pswitch;
    ...
    }else if(D(i,j)==true)
    {
      ...
      bool nolink = JorisFunctionRandomNumber() < pswitch; // nolink = dist(gen) < pswitch;
...
IntegerVector sampleNL(IntegerVector G, NumericVector eta, NumericVector phi)
{
  ...
  for(int i = 0; i < G.length(); i++) 
  {
    ...
    { 
      //All equally likely
      H(i) = JorisFunctionRcppSample(choice_set, eta);
    }else
    {
      ...
      H(i) = JorisFunctionRcppSample(choice_set, prob);
    }
  }
  return H;
}
                               ...
                               NumericVector phi, NumericVector eta)
{
  ...
  for(int i = 0; i < GA.length(); i++) 
  {
    ...
    if(GA(i)==0 && GB(i)==0)
    {
      ...
      H(i) = JorisFunctionRcppSample(choice_index, prob);
