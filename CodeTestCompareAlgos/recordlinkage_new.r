...
loglik = function(LLL, LLA, LLB, D, sumRowD, sumColD, gamma)
{
...
simulateH = function(data, D, sumRowD, sumColD, eta, omega, phi)
{
  ...
  for(k in 1:length(data$nvalues))
  {
    ...
    # print(head(truepivsA[nonlinkedA,k]))

    if(nrow(links)>0)
    {
      # start1 = Sys.time()
      #Choicesets and comparisons (this is just some bookkeeping, i dont want to do all this in RCPP)
      choice_set = expand.grid(1:data$nvalues[k], 1:data$nvalues[k])
      # print(head(choice_set,15))
      choice_equal = as.numeric(choice_set[,2] == choice_set[,1])
      # print(head(choice_equal,15))
      #choice_index = 1:nrow(choice_set)
      #Prepare P(HA)
      eta_choice = eta[[k]][choice_set[,1]]

      times = data$regTimeB[links[,2]] - data$regTimeA[links[,1]]
      # end1 = Sys.time()
      # print("simulateH, 1")
      # print(end1-start1)
...
simulateD = function(data, D, sumRowD, sumColD, truepivsA, truepivsB, gamma, eta, omega, phi)
{
  ...
  times = data$regTimeB[select[,2]] - data$regTimeA[select[,1]]
  ...
  LL0 = loglik(LLL=LLL, LLA=LLA, LLB=LLB, D=D, sumRowD=sumRowD, sumColD=sumColD, gamma=gamma)
  ...
  if (round(Dsample$loglik, digits = 3) != round(loglik( LLL=LLL, LLA=LLA, LLB=LLB, D=Dsample$D, sumRowD=Dsample$sumRowD, sumColD=Dsample$sumColD, gamma=pLink), digits = 3))
  {
    print( "Sanity check failed" )
    print( "Log likelihood associated with new Delta:" )
    print( Dsample$loglik )
    print( "Log likelihood computed on the new Delta:" )
    print( loglik( LLL=LLL, LLA=LLA, LLB=LLB, D=Dsample$D, sumRowD=Dsample$sumRowD, sumColD=Dsample$sumColD, gamma=pLink) )
  }
...
stEM = function(data, nIter, nBurnin, MStepIter, trace=1)
{
  ...
  for(iter in 2:nIter)
  {
    #print("ITER STEM")
    tijdM = Sys.time()
    D = Matrix(0, nrow=nrow(A), ncol=nrow(B), sparse=TRUE)
    sumRowD = rowSums(D)
    sumColD = colSums(D)
    #############################################################################
    ...
    for(j in 1:nBurnin)
    {
      # print("ITER GIBSS")
      # print("call simuH")
      newTruePivs = simulateH(data=data, D=D, sumRowD=sumRowD, sumColD=sumColD, eta=eta, omega=omega, phi=phi)
      truepivsA = newTruePivs$truepivsA
      truepivsB = newTruePivs$truepivsB
      # print("call simuD")
      simuD = simulateD(data=data, D=D, sumRowD=sumRowD, sumColD=sumColD, truepivsA=truepivsA, truepivsB=truepivsB, gamma=gamma, eta=eta, omega=omega, phi=phi)
      D = simuD$D
      sumRowD = simuD$sumRowD
      sumColD = simuD$sumColD
      
      # print(which(D==1, arr.ind=TRUE))
    ...
    # Vomega = lapply(data$nvalues, function(x) c())
    Vphi = lapply(data$nvalues, function(x) c())
    
    createOmegadata <- function(ncoef, stable){
      if (!stable){
        return ( data.frame(matrix(nrow = 0, ncol = ncoef)) )
      }
    }
    ...
    for(j in 1:MStepIter)
    {
      ...
      for(k in 1:length(data$nvalues))
      {
        if(!data$pivs_stable[k])
        {
          ...
          Vomega[[k]] = rbind(Vomega[[k]], 
                              Vtmp_k)
          # Vomega[[k]] = rbind(Vomega[[k]], 
          #                     data.frame(equal = truepivsA[links[,1],k] == truepivsB[links[,2],k],
          #                                time  = times, 
          #                                covA  = data$XA[, pSameH.varA[[k]], drop=FALSE], 
          #                                covB  = data$XB[, pSameH.varB[[k]], drop=FALSE]))
        }
      }
  ...
  for(m in 1:1000)
  {
    newTruePivs = simulateH(data=data, D=D, sumRowD=sumRowD, sumColD=sumColD, eta=eta_avg, phi=phi_avg, omega=omega_avg)
    truepivsA = newTruePivs$truepivsA
    truepivsB = newTruePivs$truepivsB
    simuD = simulateD(data=data, D=D, sumRowD=sumRowD, sumColD=sumColD, truepivsA=truepivsA, truepivsB=truepivsB, gamma=gamma_avg, eta=eta_avg, omega=omega_avg, phi=phi_avg)
    D = simuD$D
    sumRowD = simuD$sumRowD
    sumColD = simuD$sumColD
    Delta = Delta + D
  ...
  # ###### Final Graphic of Delta
  # 
  # gamma_avg = apply(gamma.iter, 2, function(x) mean(x))
  # eta_avg = lapply(eta.iter, function(x) apply(x, 2, mean))
  # phi_avg = lapply(phi.iter, function(x) apply(x, 2, mean))
  # w_avg = lapply(w.iter, function(x) apply(x, 2, mean))
  # 
  # 
  # image(x=1:nrow(data$pivsB), y=1:nrow(data$pivsA), z=t(as.matrix(Delta)), xlab="obs. in B", ylab="obs. in A") # gray.colors(10)
  # title(main = "Estimated linkage matrix representing the data", font.main = 4)
  # print("Number of linked records in the linkage matrix (with proba > 0.5)")
  # print(sum(Delta>0.5))
