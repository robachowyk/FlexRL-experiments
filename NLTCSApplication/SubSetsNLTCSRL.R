...
newDirectory = sprintf("NLTCS Subsets Regions %s", Sys.time())
...
unique_reg = sort(unique(c(A$reg,B$reg)))

for(i in 1:length(unique_reg)){
  
  B = read.table("encodedNLTCS1982.txt")
  A = read.table("encodedNLTCS1994.txt")
  
  region = unique_reg[i]
  
  A = A[ !is.na(A$reg) & (A$reg==region), ]
  B = B[ !is.na(B$reg) & (B$reg==region), ]
  ...
  write.csv(df, file.path(newDirectory, sprintf("datasetNLTCS_recapstory_%s.csv", region)))
  ...
  write.csv(results, file.path(newDirectory, sprintf("datasetNLTCS_results_%s.csv", region)))
